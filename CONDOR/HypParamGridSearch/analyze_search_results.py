...
def collect_results(models_dir):
    ...
    model_dirs = glob(os.path.join(models_dir, "Raw_Transformer_Assignment_h*"))
    ...
    for model_dir in model_dirs:
        ...
        try:
            ...
            trainable_params = int(history['trainable_params']) if 'trainable_params' in history else None
...
def plot_grid_search_results(df, output_dir):
    ...
    if df['best_val_acc'].notna().any():
        ...
        plt.xlabel('Hidden Dimension')
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'grid_search_accuracy_heatmap.png'), dpi=150)
        plt.close()
    ...
    model_labels = [f"h{row['hidden_dim']}_l{row['num_layers']}" 
...
def main():
    ...
    OUTPUT_DIR = os.path.join(ROOT_DIR, "plots", "grid_search_analysis")
    
    print("Collecting results from grid search...")
    results_df = collect_results(MODELS_DIR)
    ...
    print("\nTop 5 models:")
    display_cols = ['hidden_dim', 'num_layers', 'trainable_params', 'best_val_loss', 'best_val_acc', 'best_epoch']
    ...
    if results_df['trainable_params'].notna().any():
        ...
        results_df['loss_per_mparam'] = results_df['best_val_loss'] / (results_df['trainable_params'] / 1e6)
        most_efficient = results_df.loc[results_df['loss_per_mparam'].idxmin()]
        print(f"  Config: h{most_efficient['hidden_dim']}_l{most_efficient['num_layers']}")

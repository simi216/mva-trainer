...
MAX_JETS = 4
DIR_NAME = "/data/dust/group/atlas/ttreco/plots_reg_hpo/"


def main(argv):
    if len(argv) != 8:
        print(
            "Usage: python regression_transformer_script.py <hidden_size> <attention_blocks> <ff_layers> <attention_heads> <dropout_rate> <regularization_lambda> <regression_activation> <alpha>"
            "\nExample: python regression_transformer_script.py 16 3 2 4 0.1 0.001 0.5"
        )
        sys.exit(1)
    ...
    JetMatcher = RegressionKFold(
        SequentialRegression, data_processor=DataProcessor, n_folds=5, n_splits=4
    )
    JetMatcher.build_models(
        ...
        ff_layers = 1,
        dropout_rate=0.05,
        regression_dim = hidden_size,
        regression_attention_stacks = attention_blocks,
        regression_ff_layers = ff_layers,
        regularization_lambda = 0.001,
    )
    JetMatcher.compute_sample_weights(alpha=alpha)

    JetMatcher.compile_models(
        lambda_excl=0, optimizer=keras.optimizers.Adam(learning_rate=0.002)
    )

    JetMatcher.train_models(epochs=100, batch_size=1024, verbose=0, weight="sample")
    ...
    with open(PLOTS_DIR + "accuracies.dat", "w") as f:
        ...
        for i in range(JetMatcher.n_regression_targets):
            f.write(f" {regression_accuracy[i]} {regression_accuracy_err[i]}")
        f.write("\n")
